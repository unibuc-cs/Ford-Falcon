        ...
        if ($friend_result->num_rows > 0) {
            ...
            if ($check_result->num_rows == 0) {
                ...
                if ($stmt->execute()) {
                    ...
                    $error_message = "Error adding you to the calendar.";
                }
            } else {
                $error_message = "You are already a member of this calendar.";
            }
        } else {
            $error_message = "You must be friends with the creator of the calendar to add it.";
        }
    } else {
        $error_message = "Invalid calendar code.";
    }
    $stmt->close();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['delete_calendar']) && isset($_POST['calendar_id'])) {
        ...
        if ($stmt->execute()) {
            $successs_message = "Calendar deleted successfully.";
			header("Location: homepage.php");
			exit();
        } else {
            $errorr_message = "Error deleting calendar.";
        }
        $stmt->close();
    }
    ...
    if (isset($_POST['leave_calendar']) && isset($_POST['calendar_id'])) {
        ...
        $stmt->bind_param("ii", $calendar_id, $user_id);
        if ($stmt->execute()) {
            $successs_message = "You have left the calendar.";
			header("Location: homepage.php");
			exit();
        } else {
            $errorr_message = "Error leaving the calendar.";
        }
        $stmt->close();
    }
}
    ...
    while ($row = $calendar_result->fetch_assoc()) {
        ...
        if ($is_creator) {
            echo "<form method='post' action='' style='display:inline;'>
                    ...
                    <button type='submit' name='delete_calendar' class='action-button'>Delete</button>
                  </form>";
        } else {
            echo "<form method='post' action='' style='display:inline;'>
                    <input type='hidden' name='calendar_id' value='" . htmlspecialchars($row['calendar_id']) . "'>
                    <button type='submit' name='leave_calendar' class='action-button'>Leave</button>
                  </form>";
        }
    }
} else {
...
<div id="codeFormContainer">
    <form method="post" action="">
        ...
        <button type="submit" id="addCalendarButton">Add Calendar</button>
    </form>
</div>
